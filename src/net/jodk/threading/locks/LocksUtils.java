...
public class LocksUtils {
     ...
     */
    public static boolean checkNotLocked(final InterfaceCheckerLock lock) {
        if (lock.isLocked()) {
     ...
     */
    public static boolean checkNotHeldByAnotherThread(final InterfaceCheckerLock lock) {
     ...
     */
    public static boolean checkHeldByCurrentThread(final InterfaceCheckerLock lock) {
        if (!lock.isHeldByCurrentThread()) {
            if (lock.isLocked()) {
     ...
     */
    public static boolean checkNotHeldByCurrentThread(final InterfaceCheckerLock lock) {
        if (lock.isHeldByCurrentThread()) {
            throw new IllegalStateException(
                    "Lock held by current thread ["
                    +LangUtils.toString(Thread.currentThread())
